...
- Marks a link as expired when its page 404s or redirects to a listing page (the event has been removed and is in the past)
- Serves a web overview of a channel's upcoming events at `GET /board/{channel_id}` (ordered by date, unknown-date events grouped at the top, expired/past events hidden)
