...
- `__main__.py` picks the platform from `CONCERTO_PLATFORM` (`slack` default, or `discord`), lazily imports that module's `create_app` (so a Slack deploy needn't install the optional `discord` extra), and runs it under uvicorn (`HOST`/`PORT`).
...
**Discord layer — `discord_bot.py`:** `DiscordBotService(BoardService)` wraps a `discord.py` gateway client (needs the optional `discord` extra). `on_message` → `apply_message`; `on_raw_reaction_add/remove` fetch the message and `apply_reactions`; a `!concerto rebuild` text command scans `channel.history` and `replace_board`. `_reaction_name` maps Discord unicode emoji onto the core's neutral shortcodes (custom server emoji keep their own name); `_normalize_reactions` fetches reaction users only for tracked emoji. `message_url` builds a `discord.com/channels/...` link via the cached guild. There is **no** join-time backfill (unlike Slack) — the rebuild command covers it.
